...
class ApiFatoorahController extends Controller
{
    ...
    public function pay(Request $request){
       ...
       $invoiceItems[] = [
          ...
          $data = ['method' => 'الدفع بالانترنت','order_id'=>$order->id];
     Mail::send('front.mail',$data, function($message) {
               $message->to('kanaan.el.bhissy@gmail.com');
               $message->subject('تم اضافة طلب جديد');
            }); 
       ...
       return $this->fatoorahservice->sendPayment($data);
        ...
        $SMS_Msg="شكرا لكم من خروف البيت https://www.kharufalbayt.com.sa/report/".$order->id;
    ...
    \Cart::session(auth('user')->user()->mobile)->clear();
     //send Email
     $data = ['method' => 'الدفع عند الاستلام','order_id'=>$order->id];
     Mail::send('front.mail',$data, function($message) {
               $message->to('kanaan.el.bhissy@gmail.com');
               $message->subject('تم اضافة طلب جديد');
            });
    ...
    public function mpaymentcallback(Request $request){
        ...
        $order->status=1;
        $order->save();
        return redirect()->route('monlinesuccess');
        //dd($response['Data']);
    }
